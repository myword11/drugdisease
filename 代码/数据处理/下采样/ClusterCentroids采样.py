...
def group_downsampling_with_ClusterCentroids(df, group_sizes, feature_columns, n_init=10, voting='auto'):
    ...
    for group, size in zip(df['drug_group'].unique(), group_sizes):
        ...
        X_resampled, y_resampled = Cluster_Centroids(group_data_features, group_data_labels, sampling_strategy, n_init,
                                                     voting)
...
def save_resampled_data(resampled_data):
    try:
        ...
        resampled_data.to_csv("D:/a毕设/数据/未归一化/disAssociation_ClusterCentroids.csv", index=False)
...
if __name__ == '__main__':
    ...
    try:
        print("开始分组下采样...")
        resampled_data, resampled_labels = group_downsampling_with_ClusterCentroids(final_reordered, group_sizes,
                                                                                    feature_columns, n_init=10,
                                                                                    voting='auto')
